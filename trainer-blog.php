<?php
function trainer_divi_child_theme_setup() {
    if ( class_exists('ET_Builder_Module')) {

  
class trainer_ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	function init() {
		$this->name       = esc_html__( 'TrainerGrid', 'et_builder' );
		$this->plural     = esc_html__( 'TrainerGrid', 'et_builder' );
		$this->slug       = 'et_pb_trainerblog';
		$this->vb_support = 'on';
						...
						<?php if ( 'off' === $args['fullwidth'] || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
							...
							<?php
								...
								if ( 'on' === $args['show_content'] ) {
									...
									if ( has_excerpt() ) {
										//the_excerpt();
										$trainerbio = get_field('trainer_bio');
										echo $trainerbio;
									} else {
										if ( '' !== $post_content ) {
		...
		if ( have_posts() ) {
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) { ?>
				...
				<?php
					...
					echo '<div class="post-content trainerbio">';
					global $et_pb_rendering_column_content;

					//$post_content = et_strip_shortcodes( et_delete_post_first_video( get_the_content() ), true );
					$trainerbio = get_field('trainer_bio');
					$post_content = et_strip_shortcodes( et_delete_post_first_video( $trainerbio ), true );
					$et_pb_rendering_column_content = true;
 ...
 $et_builder_module_event = new trainer_ET_Builder_Module_Blog;

    add_shortcode( 'et_pb_trainerblog', array($et_builder_module_event, '_shortcode_callback') );
    }
}
add_action('et_builder_ready', 'trainer_divi_child_theme_setup');
