<?php
function member_divi_child_theme_setup() {
    if ( class_exists('ET_Builder_Module')) {

  
class member_ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	function init() {
		$this->name       = esc_html__( 'MemberGrid', 'et_builder' );
		$this->plural     = esc_html__( 'MemberGrid', 'et_builder' );
		$this->slug       = 'et_pb_memberblog';
		$this->vb_support = 'on';
		...
		if ( have_posts() ) {
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) { ?>
				...
				<?php
					...
					$memberintro = get_field('intro');
					if ($memberintro) {
					$post_content = et_strip_shortcodes( et_delete_post_first_video( $memberintro ), true );
						}
						else {
							$post_content = et_strip_shortcodes( et_delete_post_first_video( get_the_content() ), true );
							}
 ...
 $et_builder_module_event = new member_ET_Builder_Module_Blog;

    add_shortcode( 'et_pb_memberblog', array($et_builder_module_event, '_shortcode_callback') );
    }
}
add_action('et_builder_ready', 'member_divi_child_theme_setup');